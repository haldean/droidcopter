 ...
 * @version $Id: SectorSelectAndSave.java 13196 2010-03-10 07:40:57Z garakl $
 */
public class SectorSelectAndSave extends ApplicationTemplate
{
    public static class AppFrame extends ApplicationTemplate.AppFrame
    {
                ...
                public void run()
                {
                    try
                    {
                        ...
                        if (null != elevations)
                        {
                            ...
                            jd.setVisible(false);
                            JOptionPane.showMessageDialog( wwjPanel, "Elevations saved into the " + saveToFile.getName());
                        }
                        else
                        {
                            jd.setVisible(false);
                            JOptionPane.showMessageDialog( wwjPanel, "Attempt to save elevations to the " + saveToFile.getName() + " has failed.");
                        }
                    }
                    catch (Exception e)
                    {
        ...
        public void doSaveImage()
        {
            ...
            final File saveToFile = this.selectDestinationFile("Select a destination GeoTiff file to save the image", "image");
        ...
        private int[] adjustSize( Sector sector, int desiredSize )
        {
            ...
            if( null != sector && desiredSize > 0 )
            {
                ...
                int maxSize = (int) ((min == 0d) ? desiredSize : ( (double)desiredSize * max / min));

                if( dLon > dLat )
                {
                    size[1] = desiredSize;  // height
                    size[0] = maxSize;      // width
                }
                else
                {
                    size[0] = desiredSize;  // width
                    size[1] = maxSize;      // height
                }
            }
