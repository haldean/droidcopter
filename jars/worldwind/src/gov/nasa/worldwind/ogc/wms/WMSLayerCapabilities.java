...
public class WMSLayerCapabilities extends AbstractXMLEventParser
{
    ...
    public void resolveAttributes(WMSLayerCapabilities parentLayer)
    {
        ...
        if (this.getCascaded() == null && this.parent != null)
            this.setCascaded(this.parent.getCascaded());
        else
            this.setCascaded(false);

        if (this.queryable == null && this.parent != null)
            this.setQueryable(this.parent.isQueryable());
        else
            this.setQueryable(false);

        if (this.noSubsets == null && this.parent != null)
            this.setNoSubsets(this.parent.isNoSubsets());
        else
            this.setNoSubsets(false);

        if (this.opaque == null && this.parent != null)
            this.setOpaque(this.parent.isOpaque());
        else
            this.setOpaque(false);

        if (this.getFixedWidth() == null && this.parent != null)
            this.setFixedWidth(this.parent.getFixedWidth());
        else
            this.setFixedWidth(0);

        if (this.getFixedHeight() == null && this.parent != null)
            this.setFixedHeight(this.parent.getFixedHeight());
        else
