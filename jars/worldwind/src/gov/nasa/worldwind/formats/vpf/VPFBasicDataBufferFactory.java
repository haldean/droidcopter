...
/**
 * @author dcollins
 * @version $Id: VPFBasicDataBufferFactory.java 12476 2009-08-18 07:56:38Z dcollins $
 */
public abstract class VPFBasicDataBufferFactory implements VPFDataBufferFactory
{
    ...
    protected static class TextDataBuffer implements VPFDataBuffer
    {
        ...
        public boolean hasValue(int index)
        {
            ...
            {
                return this.buffer.getSubLengthBuffer().get(index) > 0;
            }
            // Fixed length text is null if it equals "N/A".
            else
            {
                return !isNoValueText(this.buffer.getSubString(index));
            }
        }
    ...
    protected abstract static class AbstractVecReader implements VecReader
    {
        ...
        protected BufferStateHandler bsh = new BufferStateHandler();
    ...
    protected static class VecDataBuffer implements VPFDataBuffer
    {
        ...
        public VecDataBuffer(VecReader reader, int coordsPerElem, BufferFactory bufferFactory, int numRows,
            int elementsPerRow)
        {
            int bufferLength = elementsPerRow;
            if (bufferLength < 1)
                bufferLength = 1;

            this.reader = reader;
            this.buffer = new CompoundVecBuffer(coordsPerElem, bufferFactory, 1 + numRows, bufferLength);
        ...
        public Object get(int index)
        {
            return this.buffer.getSubBuffer(index);
        }
        ...
        public void read(ByteBuffer byteBuffer, int length)
        {
            ...
            if (vecBuffer != null)
            {
                this.buffer.appendSubBuffer(vecBuffer);
            }
            else
            {
                this.buffer.appendEmptyBuffer();
