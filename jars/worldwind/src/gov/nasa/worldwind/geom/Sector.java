 ...
 *
 * @author Tom Gaskins
 * @version $Id: Sector.java 13350 2010-04-30 03:35:45Z tgaskins $
 * @see Angle
 */
public class Sector implements Cacheable, Comparable<Sector>, Iterable<LatLon>
{
    ...
    /**
     * Creates a new Sector which bounds the specified projected bounding rectangle, according to the specified
     * projection parameters. The coordinates are assumed to be non-geographic. If the specified parameter list
     * describes a UTM projection, this is equivalent to calling {@link #fromUTMBounds(double, double, double, double,
     * gov.nasa.worldwind.avlist.AVList)}, where the X and Y coordinates correspond to the UTM easting and northing,
     * respectively. This returns null if the parameter list describes an unrecognized projection.
     *
     ...
     *
     * @return a new bounding Sector bounding the specified projected rectangle.
     *
     * @throws IllegalArgumentException if the parameter list is null.
     */
    public static Sector fromProjectedBounds(double minX, double maxX, double minY, double maxY, AVList params)
    {
        if (params == null)
        {
        ...
        String projectionKey = (String) params.getValue(WorldFile.WORLD_FILE_PROJECTION);
        if (projectionKey == null)
            return null;

        if (projectionKey.equalsIgnoreCase("utm"))
        {
            return fromUTMBounds(minX, maxX, minY, maxY, params);
        }
        else
        {
            return null;
    ...
    /**
     * Creates a new Sector which bounds the specified UTM bounding rectangle, according to the specified UTM projection
     * parameters. The parameter list must define the UTM zone and hemisphere in the parameters {@link
     * gov.nasa.worldwind.formats.worldfile.WorldFile#WORLD_FILE_ZONE} and {@link gov.nasa.worldwind.formats.worldfile.WorldFile#WORLD_FILE_HEMISPHERE},
     * respectively.
     *
     ...
     *
     * @return a new bounding Sector bounding the specified UTM rectangle.
     *
     * @throws IllegalArgumentException if the parameter list is null, or if it does not contain the parameters {@link
     *                                  gov.nasa.worldwind.formats.worldfile.WorldFile#WORLD_FILE_ZONE} and {@link
     *                                  gov.nasa.worldwind.formats.worldfile.WorldFile#WORLD_FILE_HEMISPHERE}.
     */
    public static Sector fromUTMBounds(double minEasting, double maxEasting, double minNorthing, double maxNorthing,
        AVList params)
    {
        if (params == null)
        {
