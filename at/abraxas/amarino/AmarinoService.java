...
import org.haldean.chopper.pilot.R;
...
public class AmarinoService extends Service {
	...
	public void onCreate() {
		...
		try {
			...
			Log.i("amarino", "logfile successfully created");
		}
		catch (IOException e) {
			Log.e("amarino", "Cannot open logfile");
			e.printStackTrace();
		}
		Amarino.setAmarinoService(this);
		Log.i(TAG, "amser oncreate Thread ID " + Thread.currentThread().getId());
	}
	...
	public void onDestroy() {
		Amarino.setAmarinoService(null);
		super.onDestroy();
		...
		try {
			if (logfile != null) {
				logfile.close();
				Log.i("amarino", "logfile successfully closed");
			}
		}
		catch (IOException e) {
	...
	public IBinder onBind(Intent intent) {
		...
		if (binder instanceof AmarinoServiceBinder) {
			Log.i(TAG, "returning an amserbind");
		}
		else {
			Log.i(TAG, "not returning amserbind");
		}
		return binder;
	}
	...
	public class AmarinoServiceBinder extends Binder {
		public AmarinoService getService() {
			Log.i(TAG, "binder returning service");
